...
datadir = '/Users/njp60/Documents/code/mutualfundbackend/funddata/data/Canera Robeco Mutual Fund/'
output_file = 'canerarobeco_mutual_fund_portfolio.xlsx'
AMC_NAME = "Canera Robeco Mutual Fund"
    ...
    for i, df in enumerate(df_generator):
        ...
        if len(data.columns) >= 6 and return_df_if_has_isin(data) is not None:
            ...
            if header_row_idx is not None:
                ...
                if 'Coupon Rate' in data.columns or 'Rate (%)' in data.columns:
                    coupon_col = 'Coupon Rate' if 'Coupon Rate' in data.columns else 'Rate (%)'
                    data['Yield'] = pd.to_numeric(data[coupon_col].str.replace('%', ''), errors='coerce').fillna(0)
                    data['Type'] = data['Yield'].apply(lambda x: 'Debt or related' if x != 0 else 'Equity or Equity related')
                else:
                    # Check if there's a separate column indicating instrument type
                    if 'Instrument Type' in data.columns:
                        data['Type'] = data['Instrument Type'].apply(
                            lambda x: 'Debt or related' if 'debt' in str(x).lower() else 'Equity or Equity related'
                        )
                        data['Yield'] = data['Type'].apply(lambda x: 0 if x == 'Equity or Equity related' else None)
                    else:
                        data['Yield'] = 0
