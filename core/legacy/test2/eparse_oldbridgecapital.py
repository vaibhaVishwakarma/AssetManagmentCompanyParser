...
datadir = '/Users/njp60/Documents/code/mutualfundbackend/funddata/data/Old Bridge Capital/'
output_file = 'oldbridge_mutual_fund_portfolio.xlsx'
AMC_NAME = "Old Bridge Capital"
...
def extract_fund_data(df_generator):
    ...
    for i, df in enumerate(df_generator):
        ...
        # Old Bridge Capital specific logic
        if len(data.columns) >= 6 and return_df_if_has_isin(data) is not None:
            ...
            if header_row_idx is not None:
                ...
                required_cols = ["Name of Instrument", "ISIN", "Industry", "Quantity", "Market Value", "% to Net Assets"]
                if all(col in data.columns for col in required_cols):
                    data = data.dropna(subset=["ISIN", "Name of Instrument"])
                    data["Type"] = "Equity or Equity related"  # Old Bridge Capital primarily deals with equity
                    print(data.head(50))
                    return data
    return None
